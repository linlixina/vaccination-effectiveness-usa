...
state_mapping_data <- read.csv("usa/data/states_mapping_file.tsv", 
...
end <- as.Date("2021-11-18")
ends <- format(lubridate::as_date(first : end), "%m-%d-%Y.csv")
jhu_urls <- paste0(jhu_url, ends)

message("Downloading raw data from JHU")
# download them
jhu_dfs <- lapply(jhu_urls, data.table::fread)
...
varnames <- names(cleaned_dfs[[1]]) # variable names
vattr <- purrr::map_chr(varnames, ~class(cleaned_dfs[[1]][[.x]])) # variable attributes

for (i in seq_along(cleaned_dfs)) {
  # assign the same attributes of list 1 to the rest of the lists
  for (j in seq_along(varnames)) {
    if (varnames[[j]]  %in% names(cleaned_dfs[[i]])) {
      class(cleaned_dfs[[i]][[varnames[[j]]]]) <- vattr[[j]]
    } 
  }
}
...
# group and bind
jhu_df <- as.data.frame(data.table::rbindlist(cleaned_dfs, use.names= TRUE,fill = TRUE))
nms <- names(jhu_df)
start_nms <- c("Admin2","Province_State")
