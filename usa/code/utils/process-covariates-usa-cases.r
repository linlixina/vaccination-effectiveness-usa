...
process_covariates <- function(states, mobility, intervention = NULL, 
  ...
  pad_serial.interval <- data.frame(
    "X"=(length(serial_interval$fit)+1):477,
    "fit"=rep(1e-17, 377)
  )
  ...
  for(State in states) {
    ...
    vacci_rate = rep(0,num_days_sim) 
    for(i in (num_days_sim-109):num_days_sim) {
      vacci_rate[i] =  vaccinations$rate[i-(num_days_sim-110)+j*110]
      vacci_rate[i] = (vacci_rate[i])/100
    }
    j=j+1
    ...
    stan_data$vacci_rate <- cbind(stan_data$vacci_rate,vacci_rate)
