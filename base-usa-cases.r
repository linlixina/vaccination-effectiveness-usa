...
testing_date <- as.Date("2021-04-10")
...
#Read vaccination
vaccinations = read.csv('usa/data/us_state_vaccinations.csv', stringsAsFactors = FALSE)

# Read google mobility
states <- read.csv(GFNAME_states, stringsAsFactors = FALSE)
names(states) <- c("sub_region_1", "code")
google_mobility <- read.csv("usa/data/Global_Mobility_Report.csv", stringsAsFactors = FALSE)
...
names(google_mobility) <- c("country_region_code", "country_region", "sub_region_1", "sub_region_2","metro", "iso", "fips","place_id",
...
mobility <- select(google_mobility, "country_region_code", "country_region", "sub_region_1", "sub_region_2",
                          "date", "retail.recreation", "grocery.pharmacy", "parks", "transitstations",
                          "workplace", "residential", "code")
...
fit = sampling(m,data=stan_data,iter=200, warmup=100, chains=2, thin=1, 
